...
class FirestoneDatabase {
    ...
    fun addCourse(db: FirebaseFirestore, courseId: Long, title: String, description: String, period: Int = 0) {
        val courseCollection = db.collection("course")
        courseCollection.add(mapOf(
    ...
    fun addPassedCourse(db: FirebaseFirestore, employeeId: Long, courseId: Long, passedDate: GregorianCalendar) {
        val passedCourseCollection = db.collection("passed")
        passedCourseCollection.add(mapOf(
            "employeeID" to employeeId,
            "courseID" to courseId,
    ...
    fun addViewedNotification(db: FirebaseFirestore, employeeId: Long, notificationId: Long) {
        ...
        viewedNotificationCollection.add(mapOf(
            "employeeID" to employeeId,
            "notificationID" to notificationId
        ))
    }
    ...
    fun updateDocument(db: FirebaseFirestore, collectionName: String, documentID: String, field: String, value: FieldValue) {
        db.collection(collectionName)
            .document(documentID)
            .update(field, value)
    }
